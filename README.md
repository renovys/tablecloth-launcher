...
**기본은 자동 조회와 알림이며, 자동 설치를 하는 것은 .wsb 설정 파일뿐입니다.** 런처 자신은 사용자가 명시적으로 켠 경우에만 자동 설치하고, macSandbox는 어떤 경우에도 자동으로 교체하지 않습니다.
...
런처 자동 설치는 **권장하지 않습니다.** 이 앱은 코드 서명이 없고 릴리스 자산의 해시·서명을 검증하지 않으므로, 배포처가 탈취되면 그대로 실행됩니다. 그럼에도 켜려면 상태 폴더를 만든 뒤 launcher-auto-install 파일의 첫 줄에 1을 적으십시오. 이 기능은 최신 런처 릴리스에 .zip 설치 자산이 있고 그 안에 버전이 일치하는 식탁보.app 번들이 있을 때만 동작합니다. 설치 자산 검증에 실패하면 기존 앱을 유지하고 다음 실행 때 일반 업데이트 알림을 표시합니다. 이 설정을 지우면 기본 알림 모드로 돌아갑니다.
